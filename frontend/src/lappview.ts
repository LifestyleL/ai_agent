...
export class LAppView {
  ...
  /**
   * マウスホイールでモデルの拡大縮小を行う。
   * @param delta ホイールのデルタ値（正=縮小、負=拡大）
   */
  public adjustScale(delta: number): void {
    const scaleFactor = delta > 0 ? 0.95 : 1.05;
    // 画面中央を基準に拡大縮小
    this._viewMatrix.adjustScale(0, 0, scaleFactor);
  }

  /**
   * Ctrl+ドラッグでモデルの表示位置を移動する。
   * @param dx スクリーン座標でのX移動量（右方向が正）
   * @param dy スクリーン座標でのY移動量（下方向が正）
   */
  public adjustTranslate(dx: number, dy: number): void {
    const { width, height } = this._subdelegate.getCanvas();
    const vdx = dx / width * 2.0;
    const vdy = dy / height * 2.0;
    this._viewMatrix.adjustTranslate(-vdx, -vdy);
  }
