...
export class LAppSubdelegate {
   ...
   */
  public onPointBegan(pageX: number, pageY: number, ctrlKey: boolean = false): void {
    if (!this._view) {
      LAppPal.printMessage('view notfound');
      return;
    }
    this._captured = true;
    this._ctrlDrag = ctrlKey;
    this._dragStartX = pageX;
    this._dragStartY = pageY;
    this._lastDragX = pageX;
    this._lastDragY = pageY;
  ...
  public onPointMoved(pageX: number, pageY: number): void {
    ...
    if (this._ctrlDrag) {
      // Ctrl+ドラッグ: モデルの表示位置を移動
      const dx = pageX - this._lastDragX;
      const dy = pageY - this._lastDragY;
      this._lastDragX = pageX;
      this._lastDragY = pageY;
      this._view.adjustTranslate(dx, dy);
      return;
    }
  ...
  public onWheel(deltaY: number): void {
    ...
    this._view.adjustScale(deltaY > 0 ? 1 : -1);
  }
  ...
  private _ctrlDrag: boolean = false;
  private _dragStartX: number = 0;
  private _dragStartY: number = 0;
  private _lastDragX: number = 0;
  private _lastDragY: number = 0;
