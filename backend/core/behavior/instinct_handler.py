...
import asyncio
import time
from core.event.event_bus import event_bus, EventType, event_handler, Event
from core.memory.memory_core import MemoryCore
from .persona import get_persona
from .drive_model import get_drive_model
    ...
    try:
        ...
        try:
            ...
            if agent_driver._global_tts_queue is not None:
                ...
                print(f"[InstinctHandler] 全局TTS队列尚未初始化，跳过本能响应")
        except queue.Full:
            print(f"[InstinctHandler] TTS队列已满，丢弃本能响应: '{text}'")
        except Exception as e:
            print(f"[InstinctHandler] 本能响应入队失败: {e}")
...
def _use_preset_response(event, urge_type, comfort_level):
    ...
    try:
        ...
        if agent_driver._global_tts_queue is not None:
            ...
            tts_item = {"text": text, "emotion": "neutral"}
            agent_driver._global_tts_queue.put(tts_item, timeout=1.0)
            print(f"[InstinctHandler] 预设本能响应已入队: '{text}'")
        else:
            print(f"[InstinctHandler] 全局TTS队列尚未初始化，跳过预设本能响应")
    except queue.Full:
        print(f"[InstinctHandler] TTS队列已满，丢弃预设本能响应: '{text}'")
    except Exception as e:
        print(f"[InstinctHandler] 预设本能响应入队失败: {e}")
